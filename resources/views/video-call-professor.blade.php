...
<head>
  ...
  @php
    $callTitle = 'Online Consultation with ' . ($professorName ?? (auth('professor')->user()->Name ?? 'Professor'));
  @endphp
...
<body>
  ...
  record.name.classList.toggle('hidden', hasVideo && !!mockMediaStream);
    ...
    function getRemoteLabel(uid){
      const key = String(uid ?? '');
      if(!key){ return IS_PROF ? STUDENT_LABEL : 'Participant'; }
      const custom = remoteCustomNames.get(key);
      if(custom){ return custom; }
      const remoteCount = Array.isArray(client?.remoteUsers) ? client.remoteUsers.length : 0;
      if(COUNTERPART_NAME && remoteCount <= 1){
        return IS_PROF ? STUDENT_LABEL : COUNTERPART_NAME;
      }
      ...
      async function connectRTM(){
        ...
        try{
          ...
          if(rtm && rtm.token){
            ...
            const otherRole = SELF_ROLE === 'professor' ? STUDENT_LABEL : (COUNTERPART_NAME || 'Professor');
